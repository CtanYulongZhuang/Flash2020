...
bin_cen = np.arange(0, 22, 0.1)

nrun0 = pp012
select_runs = [1,3,4,5,10,11,12]
#select_runs = [10,11,12]
nKH = run_num*0.0 + 1
for i in range(len(run_num)):
    if (run_num[i] in com012[0][select_runs]):
        nKH[i] = 0.0

nrun = nrun0[np.where((diameter01[nrun0] < 40) & (diameter01[nrun0] > 10) & (bl012[nrun0] == 0) & (nKH[nrun0] == 0) & (delay01[nrun0] > -500))]
...
def r_ave_weighted(delay,diameter,error,bin_cen,win_hsize):
    ...
    for i, b in enumerate(bin_cen):
        ...
        bin_dia_error[i] = np.average((diain - bin_dia[i])**2, weights = 1/errin**2)
        if (len(curr_bin_sel) < 20):
            bin_dia[i] = -999
            bin_dia_error[i] = -999
        #print(i, len(selds), bin_dia_error[i], selds.std())
...
h5 = h5py.File('PL12_selected.h5', 'w')
...
#==================range_01==================.
d_range = np.where((delay01[nrun] < 22) & (delay01[nrun] > 0))[0]
...
def plateau_model(x, a0, a1, a2, a3):
    return a0 - a1*np.exp(-x/a3)

Sopf_1,Scov_1 = curve_fit(plateau_model,delay_range,diameter_range, sigma=error_range)

continum_1 = plateau_model(delay_range, *Sopf_1)
dia_mod_1 = diameter_range-continum_1

#plt.scatter(delay_range, plateau_model(delay_range, *Sopf_1),s=1, label='Linear + sin fit' )
...
plt.scatter(delay_range, plateau_model(delay_range, *Sopf_1),s=1,color='Black', label='linear fit' )
plt.xlabel('delay [ps]')
...
select_runs = [1,3,4,5] #,10
select_runs = [10,11,12]
cgcolor = ['red', 'orange', 'green', 'blue', 'purple', 'black']

plt.figure(figsize=(14,5))
for i in range(3):
    run_n = np.where(run_num[nrun] == com012[0][select_runs[i]] )
    nrun_run = nrun[run_n]

    d_range_02 = np.where((delay01[nrun_run] > 0) & (delay01[nrun_run] < 22))[0]
